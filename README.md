...
Меняешь строку в CSV - перегенерируешь host_vars - применяешь через Ansible. Изменения видны через `git diff`, ничего не правится руками на устройстве. Один источник правды, одна точка применения, повторяемо.
